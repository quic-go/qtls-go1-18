...
func testResumption(t *testing.T, version uint16) {
	...
	getTicket := func() []byte {
		return fromClientSessionState(clientConfig.ClientSessionCache.(*lruSessionCache).q.Front().Value.(*lruSessionCacheEntry).state).sessionTicket
	}
	deleteTicket := func() {
	...
	corruptTicket := func() {
		fromClientSessionState(clientConfig.ClientSessionCache.(*lruSessionCache).q.Front().Value.(*lruSessionCacheEntry).state).masterSecret[0] ^= 0xff
	}
...
func testResumption0RTT(t *testing.T) {
	...
	_, _, err := testHandshakeWithExtraConfig(t, clientConfig, clientExtraConfig, serverConfig, serverExtraConfig)
	if err != nil {
	...
	_, hs, err := testHandshakeWithExtraConfig(t, clientConfig, clientExtraConfig, serverConfig, serverExtraConfig)
	if err != nil {
	...
	clientExtraConfig.Enable0RTT = false
	gomock.InOrder(
		cache.EXPECT().Get(gomock.Any()),
		cache.EXPECT().Put(gomock.Any(), gomock.Any()).Do(func(_ string, s *ClientSessionState) {
			state = s
...
func TestHandshakeClientEnforceALPNMatch(t *testing.T) {
	clientConn, serverConn := localPipe(t)
	server := Server(serverConn, testConfig, nil)
	...
	client := Client(clientConn, config, extraConf)
	err := client.Handshake()
	if err == nil || err.Error() != "ALPN negotiation failed. Server didn't offer any protocols" {
		t.Fatalf("Expected APLN negotiation to fail, got %v", err)
	}
	sErr := <-sErrChan
	if sErr == nil || !strings.Contains(sErr.Error(), "no application protocol") {
		t.Fatalf("Expect 'no_application_protocol' error, got %v", sErr)
	}
}
